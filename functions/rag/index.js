...
// ── Investigation diary seeding ────────────────────────────────────────────
//
// The Action Queue reads investigation diaries, not the FIR Data Store. A
// deployment where nobody has opened a diary therefore shows an empty queue —
// which is CORRECT, and useless for showing the feature works. This creates a
// small set of diaries so the statutory clocks have something to count.
//
// Three rules keep this from being the fabrication it could easily become:
//
//   1. It builds on REAL cases. Every diary is opened against an actual
//      CaseMaster row with its real crime number and registration date. Nothing
//      here invents a case that does not exist in the dataset.
//   2. It never touches a diary that already exists. An officer's real work
//      cannot be overwritten by a seed, so a case that already has a record is
//      skipped and counted as skipped.
//   3. Everything it writes is stamped `seeded: true` and attributed to
//      "seed", so a seeded diary is distinguishable from a filed one forever
//      after — in the record, in the UI, and in the audit trail.
//
// The dates are the point. Each profile positions its arrest or evidence entry
// so that the obligation engine produces a specific, checkable state — a
// custody clock with days left, one already overdue, a CCTV certificate about
// to lapse, and a case that is clean. The last one matters as much as the
// others: a queue where everything is on fire teaches nobody anything.
const SEED_PROFILES = [
  {
    key: 'custody-critical',
    note: 'Accused in custody, chargesheet not filed — the statutory clock with days left.',
    arrestedDaysAgo: 47,
    persons: [{ name: 'Ramesh Kumar', role: 'Accused', status: 'Arrested' }],
    statements: 0,
  },
  {
    key: 'custody-overdue',
    note: 'The same clock, already run out.',
    arrestedDaysAgo: 64,
    persons: [{ name: 'Suresh Naik', role: 'Accused', status: 'Arrested' }],
    statements: 1,
  },
  {
    key: 'cctv-lapsing',
    note: 'Digital evidence logged with no certificate, and the recorder about to overwrite.',
    evidence: [{ description: 'Shop CCTV, 22:00-23:00, DVR at premises', type: 'Digital', daysAgo: 26 }],
    persons: [{ name: 'Unknown accused', role: 'Accused', status: 'At large' }],
    statements: 1,
  },
  {
    key: 'quiet-case',
    note: 'An investigation that has gone silent.',
    lastDiaryDaysAgo: 96,
    persons: [{ name: 'Mahesh Gowda', role: 'Suspect', status: 'On bail' }],
    statements: 2,
  },
  {
    key: 'clean-case',
...
async function handleInvestigationSeed(req, res) {
  const app = catalystSDK.initialize(req);
  const bucket = app.stratus().bucket(CONV_BUCKET);
  const { role, caller } = await myRole(app, bucket);
  // Admin only. Seeding writes case records; it is a setup action, not
  // something an investigator should be able to trigger against live work.
  ...
  try {
    ...
    return json(res, 502, { error: `Could not read CaseMaster: ${(e && e.message) || e}` });
  }
  const flat = cases.map((r) => r.CaseMaster || r).filter((r) => r && r.CaseMasterID);
  if (!flat.length) return json(res, 200, { seeded: 0, skipped: 0, note: 'No cases in the Data Store to seed against.' });

  const existing = new Set((await loadInvIndex(bucket)).map((c) => String(c.caseMasterId)));
  const now = Date.now();
  const io = { email: 'seed', name: caller ? [caller.first_name, caller.last_name].filter(Boolean).join(' ') : 'Seed' };
  ...
  for (const profile of SEED_PROFILES) {
    // Take the next real case that has no diary yet.
    while (cursor < flat.length && existing.has(String(flat[cursor].CaseMasterID))) {
      skipped.push(String(flat[cursor].CaseMasterID));
      cursor++;
    }
    if (cursor >= flat.length) break;
    const c = flat[cursor++];
    const caseMasterId = String(c.CaseMasterID);

    const { record, created } = await createInvestigationRecord(bucket, {
      caseMasterId,
    ...
    // `created` is authoritative: the index can lag a write, and overwriting an
    // officer's real diary with demonstration entries is the one outcome this
    // must never produce.
    ...
    for (const e of profile.evidence || []) {
      await appendInvestigationEntry(bucket, caseMasterId, 'evidence', {
        description: e.description, type: e.type,
        seizureMemoRef: e.seizureMemoRef || '',
        fslStatus: 'Not sent', seeded: true, ts: at(e.daysAgo),
      }, io);
    }
    for (let i = 0; i < (profile.statements || 0); i++) {
      await appendInvestigationEntry(bucket, caseMasterId, 'statements', {
        deponent: `Witness ${i + 1}`, summary: 'Statement recorded at the scene.',
        seeded: true, ts: at(3),
      }, io);
    }
    // A diary entry last, so lastDiaryDate lands where the profile wants it.
    await appendInvestigationEntry(bucket, caseMasterId, 'diaryEntries', {
      narrative: `Seeded for demonstration — ${profile.note}`,
      seeded: true, ts: at(profile.lastDiaryDaysAgo || 2),
    }, io);

    made.push({ profile: profile.key, caseMasterId, crimeNo: record.crimeNo });
  }

  await storeAuditEvents(req, app, bucket, [{
    action: 'seed-investigations', feature: 'Investigation Diary', path: '/investigation-diary',
    detail: `${made.length} diaries seeded for demonstration`,
  }], caller);
  ...
  return json(res, 200, {
    ...
    cases: made,
    note: made.length
      ? 'Seeded diaries are stamped seeded:true and attributed to "seed". They sit on real CaseMaster rows; no case was invented and no existing diary was touched.'
      : 'Every candidate case already has a diary — nothing was changed.',
  });
}
...
async function createInvestigationRecord(bucket, payload, createdByEmail) {
  ...
  const rec = {
    ...
    // Carried so a diary created for a demonstration stays distinguishable
    // from one an officer filed — in the record, the UI and the audit trail.
    // Only ever set true by the seeder; a real create leaves it absent.
    ...(payload.seeded === true ? { seeded: true } : {}),
  };
  await bucket.putObject(invKey(caseMasterId), Buffer.from(JSON.stringify(rec)));
...
module.exports = async (req, res) => {
  try {
    ...
    if (path.endsWith('/investigation/seed')) return await handleInvestigationSeed(req, res);
